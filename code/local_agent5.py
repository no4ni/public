...
Если не нужно вызывать инструменты - не используй JSON, но если нужно - используй.
Любое действие должно сопровождаться JSON
Избегай повторения своих предыдущих ответов. Если ты уже давал такой ответ, переформулируй его или используй другой подход.
"""
...
def think_and_act(history, max_depth=6):
	...
	while depth < max_depth:
		...
		if is_repeat:
			repeat_count += 1
			if repeat_count > 3:
				return "⚠️ Не удалось сгенерировать уникальный ответ. Попробуйте переформулировать запрос."
			error_msg = "⚠️ Ответ модели повторяет предыдущий. Пожалуйста, сгенерируй новый, уникальный ответ, используя инструменты или переформулируй."
			print(color_red(error_msg))
			history.append({"role": "system", "content": error_msg})
			# Не увеличиваем depth, просто продолжаем цикл (модель перегенерирует)
			continue
