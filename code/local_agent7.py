...
# -*- coding: utf-8 -*-
"""
Локальный агент – использует общий модуль tools.py.
"""
import argparse
import os
import sys
...
# ================== ДУША (идентификация агента) ==================
def _resolve_agent_name(default: str = "Мара") -> str:
	"""
	Резолвит имя агента из аргументов командной строки.
	
	Контракт:
	  --name <имя>		   явное указание (приоритет 1)
	  <имя> как 1-й позиционный аргумент   legacy-совместимость (приоритет 2)
	  (отсутствие аргументов)			  fallback на default (приоритет 3)
	
	Возвращает валидированную строку. Пустые строки и whitespace
	отбрасываются — агент без имени не имеет смысла (имя определяет
	путь к изолированной памяти: BASE_DIR / name / memory).
	"""
	parser = argparse.ArgumentParser(
		description="Локальный ReAct-агент с персистентной памятью",
	...
	parser.add_argument(
		...
		type=str,
		default=None,
		help="Имя агента. Определяет подпапку в agents/<name>/memory/",
	)
	# Позиционный аргумент — реконструкция семантики гипотетического arg.parse("", "Мара"),
	# где второй параметр был дефолтом, а первый — пустым placeholder'ом.
	# Скрываем из --help, чтобы не засорять интерфейс легаси-костылём.
	parser.add_argument(
		"legacy_name",
		nargs="?",
		default=None,
	...
	# parse_known_args вместо parse_args: критично для агента.
	# Если процесс запущен супервизором с флагами типа --config или --daemon,
	# мы не должны падать с SystemExit — просто игнорируем неизвестное.
	args, _unknown = parser.parse_known_args()
	
	# Детерминированный приоритет с нормализацией whitespace
	candidates = [args.name, args.legacy_name, default]
	for candidate in candidates:
		if candidate is not None:
			normalized = str(candidate).strip()
			if normalized:  # отбрасываем пустые и пробельные значения
				return normalized
	
	# Недостижимо при наличии default, но защита от регрессии
	return default

# Имя агента — константа времени запуска, мутированию не подлежит.
# Используется для: путей памяти, логов, SYSTEM_PROMPT (через интерполяцию),
# идентификации в мультиагентных сценариях.
MY_NAME: str = _resolve_agent_name("Мара")

# ================== ПУТИ ==================
BASE_DIR = Path(r"E:\Jericho\agents")
...
TOOLS_DIR = BASE_DIR.parent / "private_use" / "02_code" / "tools"
...
# ================== СИСТЕМНЫЙ ПРОМПТ ==================
SYSTEM_PROMPT = "Ты - "+MY_NAME+""". У тебя есть подключение к ПК пользователя.
...
⚙️ Система:
• запуск PowerShell  — {"powershell":"dir"} или {"powershell":"cd 'C:/Users/User/OneDrive/Рабочий стол'; Get-ChildItem"}
...
• анализ экрана — {"показать_экран": {"вопрос": "что открыто?", "область": <"center" или "taskbar" или "top" или "left" или "right" или списком [x,y,w,h]>}}
...
Любое действие должно сопровождаться JSON
Используй инструмент "запомнить" для важных фактов, иначе они через 5 минут вымоются из контекста
"""

# ================== РЕЕСТР ИНСТРУМЕНТОВ ==================
# Единственный источник истины о допустимых именах tool-call'ов.
# Используется парсером для отсева мета-ключей, которые модель
# склонна изобретать ("акция", "инструмент", "шаг", "план" и т.п.).
# При добавлении нового инструмента в SYSTEM_PROMPT — синхронно добавить сюда.
KNOWN_TOOLS: frozenset[str] = frozenset({
	# Файлы
	"читать", "прочитать", "осмотреть", "переписать", "добавить",
	"формат", "анализ_изображения", "найти_файл", "анализ_папки",
	# Сеть
	"запрос", "веб_поиск", "веб_страница", "страница",
	"новости", "погода", "наука", "багфикс", "исчисление",
	"получить_прокси", "удалить_прокси", "паста",
	# Память
	"запомнить", "вспомнить", "воспоминание", "напомнить", "посмотреть_альбом",
	# Система
	"powershell", "питон", "выполнить", "время", "показать_экран",
	"энциклопедия", "телеметрия", "убраться_в",
	# Обучение / мета
	"я_молодец", "сгенерировать_пример", "посмотреть_примеры",
	"вспомнить_контекст",
	# Алиасы (legacy-совместимость)
	"list", "список_файлов", "показать", "прочитать_файл",
	"search", "поиск_веб", "time", "показать_время",
	"осмотреть_файл", "осмотреть_файл_по_ключевым",
})
...
def load_history():
	if HISTORY_FILE.exists():
		...
		for msg in history:
			...
			cleaned.append(msg)
		history = cleaned
		# РАСПАКОВЫВАЕМ кортеж из clean_history
		cleaned_history, was_duplicate = clean_history(history)
...
def save_history(history):
	# Фильтруем перед сохранением
	filtered = []
	for msg in history:
		role = msg.get("role")
		content = msg.get("content", "")
		if role == "assistant" and ('<think>' in content or '</think>' in content):
			print(color_orange(f"Не сохраняю сообщение ассистента с тегом <think>: {content[:50]}..."))
			continue
		filtered.append(msg)
	# Применяем чистку и сохраняем
	cleaned, was_save = clean_history(filtered)
	final = []
	for msg in cleaned:
		msg_copy = msg.copy()
...
def clean_history(history):
	...
	for msg in history:
		...
		key = (role, content)
		if key in seen:
			print(color_orange(f"Пропускаю глобальный дубликат {role}: {content[:50]}..."))
			was_duplicate = True
			continue
		seen.add(key)
		cleaned.append(msg)
	return cleaned, was_duplicate
...
def extract_tool_calls(obj: object, tool_calls: list[dict],
					   whitelist: frozenset[str] = KNOWN_TOOLS) -> None:
	"""
	Рекурсивно извлекает tool-call'ы из произвольной JSON-структуры.
	
	Tool-call'ом считается словарь РОВНО с одним ключом, И этот ключ
	присутствует в whitelist. Это защищает от мета-обёрток, которые
	генерирует модель:
	  {"акция": [{"инструмент": {"показать_экран": {}}, "описание": "..."}]}
	Парсер проигнорирует "акция" (нет в whitelist), "инструмент" (нет),
	"описание" (нет) — и вытащит только {"показать_экран": {}}.
	
	Мутирует `tool_calls` in-place. Возвращает None.
	"""
	if isinstance(obj, dict):
		# Словарь с одним ключом — кандидат в tool-call'ы
		if len(obj) == 1:
			key = next(iter(obj.keys()))
			if key in whitelist:
				tool_calls.append(obj)
				return  # не спускаемся глубже: вложенное — это аргументы
		# Иначе (много ключей ИЛИ ключ не из whitelist) — recurse в значения
		for value in obj.values():
			extract_tool_calls(value, tool_calls, whitelist)
	elif isinstance(obj, list):
		for item in obj:
			extract_tool_calls(item, tool_calls, whitelist)
	# Скаляры (str/int/bool/None) — терминальные узлы, игнорируем
	
def strip_code_fences(text: str) -> str:
	"""
	Удаляет markdown-обёртки ```json ... ``` (и любые другие ``` ... ```).
	
	Модель Qwen 4B часто заворачивает tool-call'ы в code-fences,
	что не мешает raw_decode (он ищет от `{`), но создаёт шум при
	fallback-парсинге и в логах. Операция идемпотентна.
	"""
	# Убираем открывающий fence с опциональным языком
	text = re.sub(r'^\s*```(?:json|JSON|javascript|js)?\s*', '', text, count=1)
	# Убираем закрывающий fence в конце
...
def local_completion(prompt, temperature=TEMPERATURE, n_predict=2048):
	...
	if temperature>0.9: time.sleep(1)
	full_response = ""
	try:
...
def execute_tool(tool_call):
	...
	if args is None:
		...
		if name in ("читать", "прочитать", "list", "список_файлов"):
			...
			args = {"запрос": args}
		elif name in ("запомнить","важное"):
			args = {"содержимое": args}
		elif name in ("вспомнить", "воспоминание"):
			args = {"ключевое_слово": args}
		elif name == "powershell":
			# Поддержка обоих форматов: строка (из промпта) и словарь (fallback)
			if isinstance(args, str):
				command = args  # Прямой формат: {"powershell": "dir"}
			elif isinstance(args, dict):
				# Fallback на случай, если модель сгенерирует {"powershell": {"команда": "dir"}}
				command = args.get("команда") or args.get("command") or next(iter(args.values()), "")
			else:
				command = str(args) if args else ""

			if not command or not command.strip():
				return "Ошибка: не указана команда PowerShell."

			# Фильтр деструктивных паттернов (PS-специфика)
			dangerous_patterns = [
				r'remove-item.*-recurse.*-force', r'del.*/f.*/s', r'rm.*-rf', r'format.*/q',
				r'diskpart', r'set-acl.*-aclobject', r'takeown', r'icacls.*/grant.*:f',
				r'reg\s+delete', r'[;|&].*remove', r'vssadmin\s+delete\s+shadows',
				r'disable-computerrestore', r'set-executionpolicy', r'invoke-command',
				r'new-object.*net\.webclient.*downloadfile', r'add-type.*assembly'
			]
			for pattern in dangerous_patterns:
				if re.search(pattern, command, re.IGNORECASE):
					return (f"⚠️ Опасная команда заблокирована.\n"
							f"Паттерн: {pattern}\nКоманда: {command[:200]}")

			try:
				# -NoProfile: изоляция от пользовательских $PROFILE
				# -NonInteractive: защита от зависания на stdin-запросах
				...
				stdout = ps_process.stdout.strip()
				stderr = ps_process.stderr.strip()

				if ps_process.returncode != 0 and stderr:
					return f"Код: {ps_process.returncode}\nОшибка:\n{stderr}\nВывод:\n{stdout}"
				return stdout if stdout else "Команда выполнена без вывода."
			except subprocess.TimeoutExpired:
				return "⚠️ Таймаут PowerShell (60 сек)."
			except Exception as e:
				return f"Ошибка подпроцесса: {e}"
	...
	if name in ("веб_поиск", "поиск_веб"):
		...
		def expand_path(p):
			if not p:
				return p
			# Заменяем %VAR% на переменные окружения Windows
			import re
			def replacer(m):
				var = m.group(1)
				return os.environ.get(var, m.group(0))
			expanded = re.sub(r'%([^%]+)%', replacer, str(p))
			# Также обрабатываем ~ (домашний каталог)
			return os.path.expanduser(expanded)

		root = expand_path(args.get("корень", os.getcwd()))
		...
		timeout = args.get("таймаут", 30)

		# Приведение списков к строке
		...
		if isinstance(search_all, list):
			search_all = " ".join(search_all)
	...
	"""
		...
		clean_script = Path(CONFIG["paths"]["public_tools_dir"]) / "Инструменты" / "cleanDrive.py"
...
def think_and_act(history, max_depth=6):
	...
	while depth < max_depth:
		...
		for msg in reversed(history):
			if msg.get("role") == "assistant":
				...
				if len(last_assistant_msgs) >= 3:
					break
		is_repeat = False
		for prev in last_assistant_msgs:
			if prev and clean_reply == prev:
				is_repeat = True
				break
		...
		if is_repeat:
			...
			if repeat_count > 3:
				return clean_reply
			error_msg = "Твой ответ "+clean_reply[:100]+" повторяет предыдущий. Попробуй по-другому."
			print(color_red(error_msg))
			history.append({"role": "system", "content": error_msg})
			save_history(history)
			continue
		...
		# Слой 1: удаляем <think>...</think> и markdown-обёртки
		text_for_parsing = re.sub(
			r'<think>.*?</think>', '', reply, flags=re.DOTALL | re.IGNORECASE
		)
		text_for_parsing = strip_code_fences(text_for_parsing)
		
		# Нормализация: {"имя"} → {"имя": {}} (валидный JSON для вызова без аргументов)
		text_for_parsing = re.sub(
			r'{\s*"([^"]+)"\s*}', r'{"\1": {}}', text_for_parsing
		)
		
		# Извлечение всех JSON-объектов из текста
		found_jsons: list[dict] = []
		...
		while idx < len(text_for_parsing):
			...
			try:
				...
				error_msg = (
					f"⚠️ Ошибка парсинга JSON (позиция {e.pos}): {e.msg}\n"
					f"Фрагмент: ...{snippet}...\n"
					f"Пожалуйста, исправь JSON и повтори вызов."
				)
				print(color_red(error_msg))
				history.append({"role": "assistant", "content": text_for_parsing})
				history.append({"role": "system", "content": error_msg})
				save_history(history)
				parse_error = True
				break
		...
		# Слой 2+3: рекурсивное извлечение tool-call'ов с whitelist-фильтром.
		# Обрабатывает любые вложенные структуры, включая:
		#   {"акция": [{"инструмент": {"показать_экран": {}}, "описание": "..."}]}
		#   {"план": {"шаг1": {"веб_поиск": {"запрос": "..."}}}}
		#   [{"читать": {"путь": "..."}}, {"время": {}}]
		tool_calls: list[dict] = []
		for obj in found_jsons:
			extract_tool_calls(obj, tool_calls)
		
		# Дедупликация с сохранением порядка (dict не хешируем — сравниваем через repr)
		...
		# Диагностика: если JSON был, но tool-call'ов не извлечено —
		# модель сгенерировала данные без вызовов (например, просто текст в {}).
		if found_jsons and not tool_calls:
			warn_msg = (
				"⚠️ JSON обнаружен, но ни один ключ не распознан как инструмент. "
				f"Извлечённые объекты: {found_jsons[:3]}. "
				"Используй ТОЛЬКО имена из системного промпта без обёрток "
				"('акция', 'план', 'инструмент' запрещены)."
			)
			print(color_red(warn_msg))
			history.append({"role": "system", "content": warn_msg})
			save_history(history)
			depth += 1
			continue
		...
		if tool_calls:
			...
			# === НОВОЕ: сохраняем ответ ассистента (без тегов <think>) в историю ===
			if clean_reply.strip():
				history.append({"role": "assistant", "content": clean_reply})
				# Также опционально можно сохранить и размышления отдельно (они уже в thoughts.json)
			...
			# Подсказка для запоминания (если осталось 2 шага)
		...
		if user_input is not None:
			with history_lock:
				...
				if reply:
					...
					if len(history) > 200:
						history = history[-200:]
					save_history(history)   # сохраняем всегда
					...
					print(color_red("⚠️ Не удалось получить ответ от модели."))
			need_prompt = True
			continue
