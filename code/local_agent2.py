...
• найти первый файл — {"найти_файл": {"имя": "<подстрока>", "кодировка": "utf-8", "корень": "<где_искать>"}} или {"найти_файл": {"всё": "<подстрока>", "кодировка": "utf-8"}} или {"найти_файл": {"содержание": "<подстрока>", "кодировка": "utf-8"}}
...
def ensure_server_running():
	...
	cmd = [
		...
		"-b", str(SERVER_BATCH),
		"-ub", str(SERVER_UB),
		"--repeat-penalty", str(SERVER_REPEAT_PENALTY),
		"--host", "127.0.0.1",
...
def get_dynamic_params(gpu_temp):
	...
	DEFAULT_TOKENS = 2048
	if gpu_temp is None:
		return DEFAULT_TEMP, DEFAULT_TOKENS
	if gpu_temp <= 30.0:
...
def execute_tool(tool_call):
	...
	if name in ("веб_поиск", "поиск_веб"):
		...
		if query.startswith('"') and query.endswith('"'):
			...
			query = query[1:-1]
		query = query.replace('"', '\"')
		# Используем ddgs_search (локальная функция)
		return ddgs_search(query)

	elif name == "найти_файл":
		search_name = args.get("имя", "")
		if not search_name:
			return "Ошибка: не указано 'имя' для поиска файла."
		...
		try:
			...
			for root, dirs, files in os.walk(current_dir):
				...
				if matches:
					break
			if not matches:
				return f"Файл с именем, содержащим '{search_name}', не найден в {current_dir} и его подпапках."
			first_match = matches[0]
			try:
				with open(first_match, 'r', encoding='utf-8') as f:
					content = f.read(1024)
			except UnicodeDecodeError:
				try:
					with open(first_match, 'r', encoding='cp1251') as f:
						content = f.read(1024)
				except Exception as e:
					content = f"(не удалось прочитать текст: {e})"
			except Exception as e:
		...
		if not question:
			return analyze_screen(region, fit=fit)  # OCR
		else:
			return analyze_screen(region, question, fit=fit)
		...
		inspect_script = CONFIG["paths"]["public_tools_dir"] / "inspect.ps1"
		ps_command = f"""
...
"""
		...
		try:
			...
			return "Ошибка: превышен таймаут (30 сек)"
		except Exception as e:
		...
		return run_sensor(file_path, lang=lang)
		...
		clean_script = CONFIG["paths"]["public_tools_dir"] / "cleanDrive.py"
		...
		return f"Неизвестный инструмент: {name}\nДоступные: время, телеметрия, читать, переписать, добавить, осмотреть, найти_файл, запомнить, вспомнить, напомнить, веб_поиск, веб_страница, запрос, формат, показать_экран, анализ_изображения, энциклопедия, питон, запуск, убраться_в, анализ_папки."

# ================== ПОИСК (DDGS) ==================
def ddgs_search(query: str) -> str:
	web_search_script = CONFIG["paths"]["public_tools_dir"] / "web_search.py"
...
def think_and_act(history, force_think=False, max_depth=5):
	...
	while depth < max_depth:
		...
		print(color_blue("=== ПОСЛЕДНИЕ СООБЩЕНИЯ В ИСТОРИИ (think_and_act) ==="))
		...
		# Оставляем только словари с одним ключом (вызовы инструментов)
		tool_calls = []
...
def input_reader():
	while not stop_event.is_set():
		try:
			...
			if not line:
				break
			print(color_grey(f"[DEBUG] Прочитано из stdin: {line.strip()}"))
			user_input_queue.put(line.strip())
		except Exception:
			break

def should_use_reasoning(user_message):
	reasoning_keywords = [
		'почему', 'анализ', 'сравн', 'оцен', 'прогноз',
		'вероятност', 'риск', 'угроз', 'стратеги', 'план',
		'комплекс', 'сложн', 'разбери', 'дум', 'ясн'
	]
	if any(kw in user_message.lower() for kw in reasoning_keywords):
		return True
...
def main():
	...
	while not stop_event.is_set():
		...
		if user_input is not None:
			print(color_grey(f"[DEBUG] Получен user_input из очереди: {user_input}"))
			with history_lock:
				history.append({"role": "user", "content": user_input})
				print(color_blue("=== ПОСЛЕДНИЕ СООБЩЕНИЯ ИСТОРИИ ПЕРЕД ОТВЕТОМ ==="))
				for msg in history[-5:]:
					print(f"{msg['role']}: {msg['content'][:100]}")
				print("===================================")

				force_think = should_use_reasoning(user_input)
				reply = think_and_act(history, force_think)
				if reply:
					print(color_yellow(f"\n{MY_NAME}: {reply}"))
