...
def format_float(val):
	# Специальная обработка точного нуля — избегаем научной нотации
	if val == 0.0:
		return "0.0"
	
	# Для очень малых величин используем научную нотацию, 
	# но защищаем мантиссу экспоненты от rstrip
	if abs(val) < 1e-6:
		s = f"{val:.10e}"
		# Убираем trailing нули только в мантиссе, до символа 'e'
		mantissa, exponent = s.split('e')
		mantissa = mantissa.rstrip('0').rstrip('.')
		# Восстанавливаем экспоненту целиком
		return f"{mantissa}e{exponent}"
	
	# Для обычных значений — фиксированная нотация
	return f"{val:.6f}".rstrip('0').rstrip('.')

def extract_singular_t_values(formula_str):
	points = set()
	pattern = r'abs\(([a-zA-Z_][a-zA-Z0-9_]*)([+-]\d+(?:\.\d+)?)?\)'
	matches = re.findall(pattern, formula_str)
	for var_name, num_str in matches:
		if num_str == '':
			points.add(0.0)
...
def read_models(filename):
	...
	while i < len(lines):
		...
		if line.startswith('# normalization:'):
			parts = line.split()
			var_name = parts[2]
			minv = float(parts[3].split('=')[1])
			maxv = float(parts[4].split('=')[1])
			current_norm[var_name] = (minv, maxv)
			i += 1
			continue
		...
		if line.startswith('#') and '(' in line and 'seed=' in line:
			if current_formula_lines and current_r2 is not None:
				...
				models.append({
					...
					'norm': current_norm.get(current_var)
				})
				current_formula_lines = []
				current_r2 = None
				current_data_ts = set()
		...
		if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*\([^)]+\)\s*=', line):
			...
			if match:
				current_input_names = [name.strip() for name in match.group(2).split(',')]
		if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*\([^)]+\)\s*=', line):
...
def process_variable(models, var_name, t, input_names):
	if not models:
		return None
	...
	if factor > 1.5:
		...
		new_lower = global_lower
		new_upper = global_upper

		if all_points:
			min_s = min(all_points)
			max_s = max(all_points)
			if t < min_s or t > max_s:
				distance = min(abs(t - min_s), abs(t - max_s))
				spread = (max_s - min_s) * 0.5
				if spread > 0:
					dist = (distance / spread) + 1
					delta = max(dist - 1.2, 0)
					if delta == 0:
						extra_mult = 2 * dist - 0.5
					else:
						pow = 3.6 * ((math.exp(-0.04 * delta))-(math.exp(-5.46 * delta)))
						extra_mult = max(1.8, dist ** pow)
					half = (new_upper - new_lower) / 2.0
					new_half = half * extra_mult
					new_lower = center - new_half
					new_upper = center + new_half

	if abs(center) > 1e-12:
		rel_lower = (center - new_lower) / abs(center) * 100.0
		rel_upper = (new_upper - center) / abs(center) * 100.0
		if abs(rel_lower - rel_upper) < 0.1:
			print(f"Относительная погрешность: ±{rel_lower:.1f}%")
		else:
			print(f"Относительная погрешность: +{rel_upper:.1f}% / -{rel_lower:.1f}%")
	else:
		print("  Центр интервала близок к нулю, относительная погрешность не определена.")
...
class IDWPolyModel:
	...
	def generate_formula(self, target_min=None, target_max=None):
		...
		if abs(amp_round) < 1e-12 or np.max(np.abs(R_train)) < 1e-9:
			...
			normal_formula = poly_str
			if target_min is not None and target_max is not None:
				scale = (target_max - target_min) / 10.0
				shift = target_min
				return f"{self.target_name}({arg_str}) = ({normal_formula}) * {format_float(scale)} + {format_float(shift)}"
			else:
				return f"{self.target_name}({arg_str}) = {normal_formula}"
		...
		for i, (xi_vec, ri) in enumerate(zip(self.X_train_global, R_train)):
			...
			ab_power = a_round * b_round
			abs_sum = " + ".join([f"{s}^{format_float(ab_power)}" for s in abs_strs])
			D = f"({abs_sum})"
			sq_sum = " + ".join(sq_strs)
			...
			# Убираем amp из C, так как он сокращается в числителе и знаменателе
			C_base = f"(cos({cos_arg}) + 1)"
			phi = f"{C_base} / {D}"
		...
		arg_str = ', '.join(self.input_names)
		normal_formula = f"{poly_str} + ({numerator}) / ({denominator})"
		if target_min is not None and target_max is not None:
			scale = (target_max - target_min) / 10.0
			shift = target_min
			return f"{self.target_name}({arg_str}) = ({normal_formula}) * {format_float(scale)} + {format_float(shift)}"
...
# -------------------------------------------------------------------
# Функция learn (обучение) – без изменений
# -------------------------------------------------------------------
def learn(datafile='data2extra.json', outfile='all_formulas.txt', no_visualize=False):
	...
	if 'bounds' in data:
		...
		bounds = [(0.1, 3.9), (0.1, 3.9), (-3.142, 3.142), (0, 15.0)]
	...
	# Нормализация целевых переменных в [0,10]
	target_norm_data = {}
	target_min = {}
	target_max = {}
	for target in target_names:
		vals = np.array(data[target])
		minv, maxv = vals.min(), vals.max()
		target_min[target] = minv
		target_max[target] = maxv
		norm_vals = (vals - minv) / (maxv - minv) * 10.0
		target_norm_data[target] = norm_vals
		data[target] = norm_vals.tolist()
	...
	for target in target_names:
		...
		for i, seed in enumerate(seeds):
			...
			formula = model.generate_formula(target_min=target_min[target], target_max=target_max[target])
			formulas_for_target.append((seed, formula, model.r2, a, b, lam, amp))

			with open(output_filename, "a", encoding="utf-8") as f:
				f.write(f"# normalization: {target} min={target_min[target]:.12e} max={target_max[target]:.12e}\n")
				t_idx = None
				...
				f.write(formula + "\n")
				f.write(f"coefR = {1 - (1 - model.r2) * (n_total - 1) / (n_total - len(input_names) - 1):.6f}\n\n")
	...
	print(f"Модель сохранена в {os.path.abspath(output_filename)}")

	if not no_visualize:
		if dim_in == 1:
			x_plot = np.linspace(X_all[:,0].min(), X_all[:,0].max(), 200).reshape(-1,1)
			for var_idx, var_models in enumerate(models):
...
def predict(t_val, formula_file='all_formulas.txt'):
	...
	if not all_models:
		print("Модель повреждена. Проведите обучение")
		return

	input_names = all_models[0].get('input_names', ['t'])
	...
	for var in models_by_var:
		iv = process_variable(models_by_var[var], var, t, input_names)
		if iv is not None:
			intervals[var] = iv
	...
	for var in sorted(intervals.keys()):
		lo, hi = intervals[var]
		lo_str = f"{lo:.10g}" if abs(lo) < 1e-3 or abs(lo) >= 1e3 else f"{lo:.6f}"
		hi_str = f"{hi:.10g}" if abs(hi) < 1e-3 or abs(hi) >= 1e3 else f"{hi:.6f}"
		print(f"{var} в интервале: ({lo_str}, {hi_str})")

# -------------------------------------------------------------------
# Главная функция
# -------------------------------------------------------------------
def main():
	parser = argparse.ArgumentParser(description="Обучение IDW-полиномиальной модели и предсказание.", formatter_class=argparse.RawTextHelpFormatter)
	group = parser.add_mutually_exclusive_group(required=True)
	group.add_argument('--learn', nargs='?', const='data2extra.json', default=None,
					   help="""Запустить обучение (можно указать файл JSON, по умолчанию data2extra.json) - формат:

	{
	  "t": [...],
	  "x": [...],
	  #Любые другие переменные
	  "need": "t", #Искомые переменные
	  "bounds": [[0.1, 3.9], [0.1, 3.9], [-3.142, 3.142], [0, 15.0]], #Настройки оракула
	  "iter": 3000
	}
	""")
	group.add_argument('--predict', type=float, metavar='T', help='Предсказать для заданного значения t')
	parser.add_argument('--novisualize', action='store_true', help='Отключить визуализацию после обучения')
	args = parser.parse_args()
	if args.learn is not None:
		learn(args.learn, no_visualize=args.novisualize)
