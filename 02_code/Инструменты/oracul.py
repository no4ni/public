...
def format_float(val):
	if abs(val) > 1e-6:
		...
		s = f"{val:.10e}".rstrip('0').rstrip('.')
		# если после удаления нулей осталась точка в конце (редко, но бывает)
		if s.endswith('.'):
			s = s.rstrip('.')
	return s

def extract_singular_t_values(formula_str):
	"""Извлекает особые точки из выражений abs(имя_переменной ± число)"""
	points = set()
	# Поддерживаем любые имена переменных: t, x, y, ...
...
def preprocess_formula(formula_str):
	s = formula_str.strip()
	# Удаляем левую часть вида "имя_переменной(аргументы) ="
...
def read_models(filename):
	...
	while i < len(lines):
		...
		# Заголовок модели: начинается с '#', содержит имя переменной и seed
		...
		# Распознаём начало формулы: любое имя_переменной(аргументы) =
		if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*\([^)]+\)\s*=', line):
			if current_formula_lines:
				# На случай, если предыдущая формула не была завершена coefR
				current_formula_lines = []
			current_formula_lines.append(line)
			i += 1
			continue
		# Строка с коэффициентом R^2
		if line.startswith('coefR'):
			...
			i += 1
			continue
		# Любая другая строка – часть формулы (многострочная формула)
		if current_formula_lines:
			current_formula_lines.append(line)
		i += 1
	...
	if current_formula_lines and current_r2 is not None:
		...
		models.append({
			...
			'singular_ts': extract_singular_t_values(formula),
			'data_ts': current_data_ts,
			'input_names': current_input_names.copy()
		})
	return models
...
def process_variable(models, var_name, t, input_names):
	...
	for i, model in enumerate(models, 1):
		...
		try:
			...
			val_str = f"{val:.6e}" if abs(val) < 1e-3 or abs(val) >= 1e3 else f"{val:.6f}"
	...
	if factor > 1.5:
		# Исходное сужение интервала по доверительному фактору
		...
		# -------------------------------------------------------------------
		# Дополнительное расширение при экстраполяции (глубокая экстраполяция)
		# -------------------------------------------------------------------
		if all_points:
			...
			if t < min_s or t > max_s:
				...
				# Характерный масштаб – половина размаха данных
				spread = (max_s - min_s) * 0.5
				if spread > 0:
					# расширение интервала для далёких от опорных точек
	...
	if abs(center) > 1e-12:
		...
		# Для симметричного случая можно вывести одну величину
...
def predict(t_val, formula_file='all_formulas.txt'):
	...
	# Берём имена входов из первой модели (предполагаем, что для всех целевых переменных они одинаковы)
...
def main():
...

	{
	  ...
	  "bounds": [[0.1, 3.9], [0.1, 3.9], [-3.142, 3.142], [0, 15.0]], #ТОЛЬКО ТАКИЕ! (Настройки самого оракула) При этом на сами переменные и их количество они НЕ влияют.
	  "iter": 3000
	}
	""")
